...
class SitToStandLogic:
    ...
    def process_frame(self, image):
        ...
        current_angle = 0; feedback = "READY"; feedback_color = (0, 255, 0); current_time_seconds = time.time() - self.start_time
...
if mode == "Webcam (Live)":
    ...
    if uploaded_file is not None:
        ...
        if not cap.isOpened():
            ...
            # Create Output File (Temp)
            output_file = tempfile.NamedTemporaryFile(delete=False, suffix='.mp4')
            
            # Logic resizes image to 640 width, so we must match that for writer
            target_w = 640
            ...
            fourcc = cv2.VideoWriter_fourcc(*'mp4v') # 'mp4v' is generic, usually works
            out = cv2.VideoWriter(output_file.name, fourcc, fps, (target_w, target_h))
